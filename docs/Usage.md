...
4. Switch identity in the session with `/role` (below). `/new` discards the session switch and re-runs the startup chain.

## `/role`

Session-only switch. Does not write `settings.json`. Does not write identity files.
